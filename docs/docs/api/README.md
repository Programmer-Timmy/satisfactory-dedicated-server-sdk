**satisfactory-dedicated-server-sdk**

***

## Enumerations

- [MinimumPrivilegeLevel](enumerations/MinimumPrivilegeLevel.md)

## Classes

- [APIError](classes/APIError.md)
- [InvalidParameterError](classes/InvalidParameterError.md)
- [SatisfactoryApi](classes/SatisfactoryApi.md)

## Interfaces

- [AdvancedGameSettings](interfaces/AdvancedGameSettings.md)
- [NewGameData](interfaces/NewGameData.md)
- [Response](interfaces/Response.md)
- [SatisfactoryApiOptions](interfaces/SatisfactoryApiOptions.md)
- [ServerOptions](interfaces/ServerOptions.md)

## Functions

- [AdvancedGameSettingsToDict](functions/AdvancedGameSettingsToDict.md)
- [serverOptionsToDict](functions/serverOptionsToDict.md)
